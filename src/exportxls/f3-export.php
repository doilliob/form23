 <?php
	...
	//================================================
	function ExportXLSX($teacher,$filename,$my_year_now,$my_year_next)	{
		//==================================================================
		...
		$objPHPExcel->getActiveSheet()->setCellValue("A36","Зам. директора по УР  _______________/Фамилия Е.В./");
		$objPHPExcel->getActiveSheet()->mergeCells("A37:L37");
		$objPHPExcel->getActiveSheet()->setCellValue("A37","Консультант учебной части  _______________/Фамилия Л.Г./");
		$objPHPExcel->getActiveSheet()->mergeCells("A38:L38");
		$objPHPExcel->getActiveSheet()->setCellValue("A38","Зам. директора по ПП  _______________/Фамилия И.П./");
		$objPHPExcel->getActiveSheet()->mergeCells("A39:L39");
		$objPHPExcel->getActiveSheet()->setCellValue("A39","Годовой учет часов, данных преподавателем, ведется учебной частью на основании записей");
		$objPHPExcel->getActiveSheet()->mergeCells("A40:L40");
		$objPHPExcel->getActiveSheet()->setCellValue("A40","в ведомости учета часов учебной работы преподавателей ( форма №2 )");
		...
		$objPHPExcel->getActiveSheet()->setCellValue( xy(1,3) , 'Данных преподавателями за '.$my_year_now.'-'.$my_year_next.' учебный год');
		$objPHPExcel->getActiveSheet()->setCellValue( xy(1,5) , 'Учет часов работы за '.$my_year_now.'-'.$my_year_next.' учебный год');
		...
		$objWriter->save(Encode($filename,'w'));
    ...
    if (isset($_GET['teacher']) ){ 
		...
		$teacher = $_GET['teacher'];
                $filename="temp/".(round(microtime(true) * 1000)).".xlsx";
				
		ExportXLSX($teacher,$filename,$my_year_now,$my_year_next);

		if (file_exists($filename)) {
    		   header('Content-Description: File Transfer');
    		   header('Content-Type: application/octet-stream');
    		   header('Content-Disposition: attachment; filename='.basename($teacher.".xlsx"));
    		   header('Expires: 0');
